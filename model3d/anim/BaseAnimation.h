...
public:
	...
	template<typename T>
	Animation* setStartFunction(T *listener,void (T::*pfn)(Node *node)){
		m_pfnStart = std::bind(pfn, listener, m_iNode);
		return this;
	}
	template<typename T>
	Animation* setCompleteFunction(T *listener,void (T::*pfn)(Node *node)){
		m_pfnComplete = std::bind(pfn, listener, m_iNode);
		return this;
	}
	template<typename T>
	Animation* setUpdateFunction(T *listener,void (T::*pfn)(Node *node,float r)){
